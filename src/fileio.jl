...
function check_atomic_mass(atom::Atom)
    if haskey(PeriodicTable.elements, atomic_number(atom))
        if mass(atom) != PeriodicTable.elements[atomic_number(atom)].atomic_mass
...
function parse_coord_line(T::Type{<:Real}, line)
    words = split(line)
    number_or_symbol = words[1]
    species = try
        number = parse(Int, number_or_symbol)
        ChemicalSpecies(number)
    catch e
        typeof(e) <: Base.ArgumentError || rethrow(e)
        ChemicalSpecies(Symbol(number_or_symbol))
    end
    position = parse.(T, words[2:4]) .* LENGTH_UNIT
    if length(words) == 7
        force = parse.(T, words[5:7]) .* FORCE_UNIT
        return Atom(species, position; force)
    else
